...
providers = organisations[:, (f.providerIdentifier, f.hasSubmittedData, f.name)]
...
# import objects
orgs = toTextCsv(organisations).encode('utf-8')
prov = toTextCsv(providers).encode('utf-8')
...
stats['component'] = dt.Frame([
  value.replace('table-','') if value == 'table-enrollment-disease-group' else value
...
emx2.importData(
  database='DashboardStats',
  table='components',
  data = toTextCsv(components)
)

emx2.importData(
  database='DashboardStats',
  table='statistics',
  data = toTextCsv(stats).encode('utf-8')
)